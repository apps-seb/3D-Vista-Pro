...
const server = app.listen(3000, async () => {
    ...
    await adminPage.waitForFunction(() => {
        const title = document.getElementById('display-title');
        return title && title.textContent !== '🔄 INICIANDO V3...';
    });
    ...
    // Wait for supabase realtime sync or just wait a bit and refresh index.html
    await indexPage.waitForTimeout(2000);

    const updatedTitle = await indexPage.locator('#display-title').textContent();
    console.log('INDEX UPDATED TITLE:', updatedTitle);
    ...
    console.log('INDEX REVERTED TITLE:', revertedTitle);
